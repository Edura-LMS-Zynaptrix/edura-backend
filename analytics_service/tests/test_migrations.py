...
def test_migration_lifecycle(engine):
    ...
    for table in expected_tables:
        assert table in existing_tables, f"Expected table '{table}' was not created by migration"
    ...
    assert len(remaining_tables) == 0, f"Some tables remained after downgrade: {remaining_tables}"
    ...
    for table in expected_tables:
        assert table in existing_tables_retry, f"Expected table '{table}' was not created on second upgrade"
